...
<head>
    ...
    <title>Update Article - NewsDesk</title>
...
if(isset($_POST['btnSave'])) {
    ...
    if(isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        if(move_uploaded_file($image['tmp_name'], $targetPath)) {
            ...
            $query = "UPDATE Articles SET ArticleName = ?, Message = ?, Email = ?, Name = ?, AuthorImage = ?, Date = ?, Category = ?, Image = ? WHERE Id = ?";
            $stmt = mysqli_prepare($con, $query);
            
            if ($stmt) {
                // Bind the parameters with the values
                mysqli_stmt_bind_param($stmt, "ssssssssi", $ArticleName, $Message, $Email, $Name, $AuthorImage, $Date, $Category, $fileName, $articleId);
                ...
                if (mysqli_stmt_execute($stmt)) {
                    ...
                    echo '<script type="text/javascript">';
                    echo 'alert("Error!! Article not updated.");';
                    echo '</script>';
                }
...
<form method="POST" action="" enctype="multipart/form-data">
        <h1 class="headings" style="width: 380px;">Update Article</h1>
